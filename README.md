...
1. `superviseur` : initialise le routage de la demande.
2. `financier` : récupère les données CSV du client et génère l'analyse financière.
3. `conformite` : interroge les documents PDF via RAG et produit l'analyse conformité.
4. `evaluateur` : combine les analyses et produit la décision finale.
5. `validation_humaine` : applique une validation humaine ou laisse la décision en attente.
...
```bash
uv run streamlit run ui/application.py
```
